...
/**
 * AdvancedList used by AdvancedSensitiveWords
 * 1.3~2x faster than arraylist and 2~3x faster than linkedlist
 * @param <T>
 */
public class AdvancedList<T> extends AbstractList<T> implements List<T>, Cloneable, Serializable {
    ...
    public AdvancedList(int initialCapacity) {
    ...
    public AdvancedList(Collection<? extends T> c) {
    ...
    public boolean addAll(Collection<? extends T> c) {
        ...
        if (remainedStorage == 0) {
            ...
            int newLen = (size >>> 1);
            int initialLen = Math.max(len, newLen);
        ...
        if (len > remainedStorage) {
            ...
            int initialLen = Math.max(newLen, remainedDataLen);
    ...
    private class Itr implements Iterator<T> {
        ...
        public void remove() {
            ...
            try {
                AdvancedList.this.remove(lastReturn);
    ...
    private class ListItr extends Itr implements ListIterator<T> {
        ...
        public void set(T t) {
            ...
            try {
                AdvancedList.this.set(lastReturn, t);
            } catch (IndexOutOfBoundsException e) {
                throw new ConcurrentModificationException();
            }
        }
    ...
    public Object clone() {
        ...
        try {
            AdvancedList<T> clone = (AdvancedList<T>) super.clone();
