...
import io.wdsj.imp.handler.encryption.PacketDecryptionHandler;
import io.wdsj.imp.handler.encryption.PacketEncryptionHandler;
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (event.getPacketType() == PacketType.Handshaking.Client.HANDSHAKE) {
            ...
            if (handshake.getNextConnectionState() == ConnectionState.LOGIN
                    && protocolVersion != Imp.SERVER_PROTOCOL_VERSION
            ...
            if (isOnlineMode()) {
                ...
                PublicKey key = Imp.KEY_PAIR.getPublic();
        ...
        //They responded with an encryption response
            ...
            Imp.WORKER_THREADS.execute(() -> {
                ...
                byte[] verifyToken = MinecraftEncryptionUtil.decryptRSA(Imp.KEY_PAIR.getPrivate(), encryptionResponse.getEncryptedVerifyToken().get());
                //Private key from the server's key pair
                PrivateKey privateKey = Imp.KEY_PAIR.getPrivate();
                ...
                digest.update(Imp.KEY_PAIR.getPublic().getEncoded());
                ...
                if (Arrays.equals(player.getVerifyToken(), verifyToken)) {
                    ...
                    try {
                        ...
                        if (connection.getResponseCode() == 204) {
                            Imp.LOGGER.info("Failed to authenticate " + player.getUsername() + "!");
                    ...
                    Imp.LOGGER.warning("Failed to authenticate " + player.getUsername() + ", because they replied with an invalid verify token!");
                    user.closeConnection();
                }
                finalEvent.cleanUp();
            });
